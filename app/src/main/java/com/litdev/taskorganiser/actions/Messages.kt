...
class Messages() {
    private val originalMessages = mapOf(
        "start" to "This is an app to organise tasks and tick them off as they are done." +
...
//            "\n► Set the task or action type, text description and optional SMS" +
    ...
    private val messages = originalMessages.toMutableMap()
    var delayReset = true
    var delayEnd = true
    ...
    fun resetMessages() {
        for ((key, value) in originalMessages) {
            messages[key] = value
        }
        saveMessages()
    }
